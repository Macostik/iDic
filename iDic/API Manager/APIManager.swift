                ...
                if case let .failure(error) = response.result {
                    if let error = error as? AFError {
                        ...
                        case .parameterEncodingFailed(let reason):
                            errorDescription = "Parameter encoding failed: " +
                            "\(error.localizedDescription)"
                            errorReason = "Failure Reason: " + "\(reason)"
                        case .multipartEncodingFailed(let reason):
                            errorDescription = "Multipart encoding failed: " +
                            "\(error.localizedDescription)"
                            errorReason = "Failure Reason: " + "\(reason)"
                        case .responseValidationFailed(let reason):
                            ...
                            case .unacceptableContentType(let acceptable, let response):
                                errorDescription = "Response content type: " + "\(response) " +
                                    "was unacceptable: " + "\(acceptable)"
